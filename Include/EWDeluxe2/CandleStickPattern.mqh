...
void findPattern(double &highBuffer[], double &lowBuffer[], double &openBuffer[], double &closeBuffer[],long &VolumeBuffer[],datetime &timeBuffer[],TradeTrigger &triggers[],Configuration &config)
{
 ...
 ArrayFree(foundPatterns);
  ...
  fBullishEngulfing(highBuffer,lowBuffer,openBuffer,closeBuffer,timeBuffer,VolumeBuffer,foundPatterns);
  if(config.TriggerBearishEngulfing)
  fBearishEngulfing(highBuffer,lowBuffer,openBuffer,closeBuffer,timeBuffer,VolumeBuffer,foundPatterns);
  if(config.TriggerThreeWhiteSoldiers)
  threeSoldiers(openBuffer,closeBuffer,timeBuffer,lowBuffer,highBuffer,VolumeBuffer,foundPatterns);
  if(config.TriggerDarkClouds)
  threeDarkCloud(openBuffer,closeBuffer,timeBuffer,lowBuffer,highBuffer,VolumeBuffer,foundPatterns);
  if(config.TriggerHammer)
  Hammer(openBuffer,closeBuffer,timeBuffer,lowBuffer,highBuffer,VolumeBuffer,foundPatterns);
  if(config.TriggerInvertedHammer)
  InvertedHammer(openBuffer,closeBuffer,timeBuffer,lowBuffer,highBuffer,VolumeBuffer,foundPatterns);
  ...
  for(int i = 0; i < ArraySize(foundPatterns); i++)
  {
      ...
      if(foundPatterns[i].position==Buy)
       ...
       tmpTrigger.triggerHigh=foundPatterns[i].patternHigh;
       tmpTrigger.triggerLow=foundPatterns[i].patternLow;
       tmpTrigger.TriggerOnPivot=false;
        tmpTrigger.PivotCount=0;
        
        tmpTrigger.triggerClosePrice = foundPatterns[i].triggerClosePrice;
...
void  fBullishEngulfing(double &highBuffer[],double &lowBuffer[], double &openBuffer[], double &closeBuffer[],datetime &timeBuffer[],long &VolumeBuffer[], CandleStickPattern &foundPatterns[])
{
   ...
   if(   closeBuffer[2]>=openBuffer[1] && openBuffer[2]<=closeBuffer[1] 
     ...
     // && (VolumeBuffer[1] > avgVolume && VolumeBuffer[2] > avgVolume))
     {
       ...
       tmpPattern.triggerClosePrice=closeBuffer[1];
...
void  fBearishEngulfing(double &highBuffer[],double &lowBuffer[], double &openBuffer[], double &closeBuffer[],datetime &timeBuffer[],long &VolumeBuffer[], CandleStickPattern &foundPatterns[])
{
    CandleStickPattern tmpPattern; 
...
void threeSoldiers(double &openBuffer[],double &closeBuffer[],datetime &timeBuffer[],double &lowBuffer[],double &highBuffer[],long &VolumeBuffer[],CandleStickPattern &foundPatterns[])
{
CandleStickPattern tmpPattern;
...
void threeDarkCloud(double &openBuffer[],double &closeBuffer[],datetime &timeBuffer[],double &lowBuffer[],double &highBuffer[],long &VolumeBuffer[],CandleStickPattern &foundPatterns[])
{
CandleStickPattern tmpPattern;
...
void Hammer(double &openBuffer[],double &closeBuffer[],datetime &timeBuffer[],double &lowBuffer[],double &highBuffer[],long &VolumeBuffer[],CandleStickPattern &foundPatterns[])
{
CandleStickPattern tmpPattern;
   ...
   && ((highBuffer[1]-openBuffer[1]) < ((openBuffer[1]-closeBuffer[1])/2) )  )
{
    tmpPattern.Type=Hammer;
       ...
       if(closeBuffer[1] > openBuffer[1])
         tmpPattern.triggerClosePrice=closeBuffer[1];
       else
         tmpPattern.triggerClosePrice=openBuffer[1];  
...
void InvertedHammer(double &openBuffer[],double &closeBuffer[],datetime &timeBuffer[],double &lowBuffer[],double &highBuffer[],long &VolumeBuffer[],CandleStickPattern &foundPatterns[])
{
CandleStickPattern tmpPattern;
